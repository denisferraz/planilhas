...
if (!empty($_FILES["xmlFile"]["name"]) && count($_FILES["xmlFile"]["name"]) > 0) {
    ...
    for ($i = 0; $i < count($file_names); $i++) {
        ...
        if (strtolower(pathinfo($file_name, PATHINFO_EXTENSION)) == "xml") {
            ...
            if ($file_handle !== FALSE) {
                ...
                foreach ($xml->LIST_G_MEMBERTYPE->G_MEMBERTYPE->LIST_G_ROOM->G_ROOM as $row) {
...
foreach ($excel_all_hotellink as $hotelLinkData) {
    ...
    if (!empty($matchingPMSData)) {
        foreach ($matchingPMSData as $pmsData) {
            ...
            $pontuacao_total = $pontuacao_total - ($pontuacao_room * 0.05);
            ...
            foreach ($excel_all_pms as &$pmsData) {
                if (
                    ...
                    $pmsData['pontuacao_total'] = $pmsData['pontuacao_total'] - ($pmsData['pontuacao_room'] * 1.05);
                }
            }
