    ...
    for ($i = 0; $i < count($file_names); $i++) {
        ...
        if (strtolower(pathinfo($file_name, PATHINFO_EXTENSION)) == "xml") {
            ...
            if ($file_handle !== FALSE) {
                ...
                //Importar Journal
                if (strpos($file_name, "foliotax_02_") !== false) {
                ...
                foreach ($xml->LIST_G_CROSS->G_CROSS->LIST_G_BILL_NO->G_BILL_NO as $row) {
                    ...
                    if((string)$row->FOLIO_TYPE != 'S_FOLIO'){
                        continue;
                    }

                    $truncBegin = (string)$row->CHAR_BILL_GENERATION_DATE;
                    $beginDate = DateTime::createFromFormat('dd-mm-yy', $truncBegin);

                        $colunaA = (string)$row->BILL_NO;
                        $colunaB = (string)$row->LIST_G_TAX_GROUP->G_TAX_GROUP->NET_AMT;
                        $colunaC = (string)$row->PAYEE_NAME_ID;
                        $colunaF = (string)$row->FOLIO_TYPE;

                        $excel_journal_data[] = [
                            'rps_num' => $colunaA,
                            'rps_valor' => $colunaB,
                            'id_guest' => $colunaC,
                            'folio_open' => $beginDate,
                            'folio_close' => $beginDate,
                            'folio_type' => $colunaF
                        ];
                    }
            }
            //Importar Arrivals
                ...
                foreach ($xml->LIST_G_GROUP_BY1->G_GROUP_BY1->LIST_G_RESERVATION->G_RESERVATION as $row) {
                    ...
                    if((string)$row->SHORT_RESV_STATUS != 'CLX'){

                    $excel_cancellations_data[] = [
                        'reserva_id' => $row->CONFIRMATION_NO,
                    ];
                ...
                $excel_arrivals_data[] = [
                    'guest_name' => (string)$row->FULL_NAME_NO_SHR_IND,
                    'checkin' => $beginDate->format('dd-mm-yy'),
                    'checkout' => $endDate->format('dd-mm-yy'),
                    'ratecode' => (string)$row->RATE_CODE,
                    'reserva_id' => (string)$row->EXTERNAL_REFERENCE,
                    'id_guest' => (string)$row->GUEST_NAME_ID,
                ];
            }
    ...
    foreach ($excel_arrivals_data as $arrival) {
        if ($arrival['reserva_id'] == $reserva_id) {
            ...
            foreach ($excel_journal_data as $journal) {
                if ($journal['id_guest'] == $id_guest && ($journal['folio_open'] >= $checkin && $journal['folio_close'] <= $checkout) && $journal['folio_type'] == 'S_FOLIO' && $journal['rps_valor'] >= $comissao['valor_rps']) {
