...
$dados_roomstatus = array_filter($dados_roomstatus_pre, function($item) use ($id) {
    return $item['id_camareira'] == $id;
});

$dados_vago_sujo = array_filter($dados_roomstatus, function($item) {
    return $item['room_status_1'] == "Vacant" && $item['room_status_2'] == "Dirty";
});

$dados_vago_limpo = array_filter($dados_roomstatus, function($item) {
    return $item['room_status_1'] == "Vacant" && $item['room_status_2'] == "Clean";
});

$dados_ocupado = array_filter($dados_roomstatus, function($item) {
    return $item['room_status_1'] == "Occupied";
});

$dados_bloqueado = array_filter($dados_roomstatus, function($item) {
    return $item['room_status_1'] == "Out of order";
});

$dados_prevista = array_filter($dados_roomstatus, function($item) {
    return $item['room_stay_status'] == "Pending depart";
});
...
<table>
<th colspan="6"><?php echo $_SESSION['camareira_'.$id]; ?></th>
<tr><td style="background-color: black" colspan="6"></td></tr>
<tr><td align="center" colspan="6">Total (<b><?php echo $quantidade_dados ?></b>) | Vago Limpo (<b><?php echo count($dados_vago_limpo) ?></b>) | Vago Sujo (<b><?php echo count($dados_vago_sujo) ?></b>) | Previstas (<b><?php echo count($dados_prevista) ?></b>) | Ocupados (<b><?php echo count($dados_ocupado)-count($dados_prevista) ?></b>) | Bloqueados (<b><?php echo count($dados_bloqueado) ?></b>)</td>
<tr><td style="background-color: black" colspan="6"></td></tr>
